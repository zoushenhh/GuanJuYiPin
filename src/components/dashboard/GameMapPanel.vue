...
watch(
  ...
  (newPos) => {
    if (newPos && mapManager.value) {
      const playerName = gameStateStore.character?.名字 || '同僚';
    ...
    if (result.success && result.worldInfo) {
      ...
      if (shouldGenerateHehuan) {
        ...
        const greyLady: NpcProfile = {
          ...
          官品: { 名称: "七品", 阶段: "圆满", 当前进度: 0, 下一级所需: 100, 突破描述: "勤政为民，晋升有望" },
          才能: { name: "卓越才能", tier: "卓越" } as any,
          ...
          背包: { 银两: { 铜钱: 5000, 碎银: 500, 纹银: 50, 元宝: 0 }, 物品: {} },
          实时关注: true,
          私密信息: {
            是否为处女: true,
            身体部位: [
