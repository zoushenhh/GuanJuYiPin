            ...
            <div class="achievement-block">
              ...
              <div v-else-if="hasValidCultivation()" class="progress-wrapper">
                <div class="progress-top">
                  <span class="progress-label">{{ t('政绩瓶颈') }}</span>
...
const tabs = computed(() => {
  const base = [
    ...
    { id: 'cultivation', label: '政绩', icon: BookOpen },
...
const getSpiritRootCultivationSpeed = (info: { 灵根?: unknown } | null): string => {
  ...
  if (typeof rootObj.施政加成 === 'number') {
    bonus = rootObj.施政加成;
  } else if (typeof rootObj.施政加成 === 'string') {
    const raw = rootObj.施政加成.trim();
    if (raw.endsWith('%')) {
      bonusIsPercent = true;
  ...
  if (typeof rootObj.施政速度 === 'number' || typeof rootObj.施政速度 === 'string') {
    return `${String(rootObj.施政速度)}x`;
  }
