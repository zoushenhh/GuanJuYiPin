...
export const useGameStateStore = defineStore('gameState', {
  ...
  actions: {
    ...
    loadFromSaveData(saveData: SaveData) {
      ...
      const sectSystem: SectSystemV2 | null = v3?.社交?.衙门 ? deepCopy(v3.社交.衙门) : null;
      let sectMemberInfo: SectMemberInfo | null = (v3?.社交?.衙门 as any)?.成员信息 ? deepCopy((v3.社交.衙门 as any).成员信息) : null;

      // 🔥 兜底：若玩家在"衙门档案领导层"中被识别为高层，但存档缺失 成员信息，则在 store 中补齐一份（仅用于 UI/保存时回写）
      try {
        if (!sectMemberInfo) {
          ...
          if (sectNameCandidate) {
            ...
            sectMemberInfo = {
              衙门名称: sectNameCandidate,
              衙门类型: ((sectProfile as any)?.类型 as any) || '政府机构',
              职位: leader.position || '普通职员',
              贡献: 0,
              关系: '友好',
              声望: 0,
    ...
    addToShortTermMemory(content: string) {
      ...
      // 添加时间前缀（使用"为官"与其他地方保持一致）
      const gameTime = this.gameTime;
      const minutes = gameTime?.分钟 ?? 0;
      const timePrefix = gameTime
        ? `【为官${gameTime.年}年${gameTime.月}月${gameTime.日}日 ${String(gameTime.小时).padStart(2, '0')}:${String(minutes).padStart(2, '0')}】`
        : '【未知时间】';

      const hasTimePrefix = content.startsWith('【为官') || content.startsWith('【未知时间】') || content.startsWith('【为历');
