...
export const useCharacterStore = defineStore('characterV3', () => {
  ...
  const saveToStorage = async (options?: {
    ...
    try {
      ...
      if (playerAttributes) {
        // 官品统一为 Realm 对象
        slot.官品 = playerAttributes.官品?.名称 || '平民';

        // 计算政绩进度百分比（从官品的当前进度获取）
        if (typeof playerAttributes.官品 === 'object' && playerAttributes.官品 !== null) {
          const realm = playerAttributes.官品 as Realm;
          if (realm.下一级所需 > 0) {
            slot.政绩进度 = Math.floor((realm.当前进度 / realm.下一级所需) * 100);
          }
        }
      }
  ...
  const saveAsNewSlot = async (saveName: string): Promise<string | null> => {
    ...
    try {
      ...
      // 计算政绩进度
      if (playerAttributes?.官品 && playerAttributes.官品.下一级所需 > 0) {
        newSlot.政绩进度 = Math.floor((playerAttributes.官品.当前进度 / playerAttributes.官品.下一级所需) * 100);
      }
  ...
  const saveToSlot = async (slotName: string): Promise<void> => {
    ...
    try {
      ...
      // 计算政绩进度
      if (playerAttributes?.官品 && playerAttributes.官品.下一级所需 > 0) {
        newSlotData.政绩进度 = Math.floor((playerAttributes.官品.当前进度 / playerAttributes.官品.下一级所需) * 100);
      }
  ...
  const rollbackToLastConversation = async () => {
    ...
    activeSlot.官品 = playerAttributes?.官品?.名称 || '平民';
...
const equipTechnique = async (itemId: string) => {
  ...
  // 3. 创建或更新政务槽位（只存储引用）
  (saveData as any).角色.政务 = {
    ...(((saveData as any).角色.政务 ?? {}) as any),
    政绩方略: {
      物品ID: item.物品ID,
      名称: item.名称,
    },
...
const unequipTechnique = async (itemId: string) => {
  ...
  // 3. 清空政务槽（如果存在的话，确保数据一致性）
  if ((saveData as any).角色?.政务?.政绩方略?.物品ID === itemId) {
    (saveData as any).角色.政务 = {
      ...(((saveData as any).角色.政务 ?? {}) as any),
      政绩方略: null,
    };
