...
/** 民心支持度系数映射表 */
const PUBLIC_TRUST_RANGES = [
  { min: 1, max: 20, minFactor: 0.1, maxFactor: 0.4, desc: '民心稀薄' },
  { min: 21, max: 40, minFactor: 0.4, maxFactor: 0.7, desc: '民心普通' },
  { min: 41, max: 60, minFactor: 0.7, maxFactor: 1.0, desc: '民心充沛' },
  { min: 61, max: 80, minFactor: 1.0, maxFactor: 1.5, desc: '民心浓郁' },
  { min: 81, max: 100, minFactor: 1.5, maxFactor: 2.0, desc: '民心极盛' },
] as const;

/** 民心基础值系数映射表 */
const PUBLIC_TRUST_BASE_RANGES = PUBLIC_TRUST_RANGES;

/** 职级晋升时间标准（游戏时间，单位：月） - 内部使用 */
...
export const RANK_PROMOTION_STANDARDS: InternalPromotionTime[] = [
  // 九品
  { 职级名称: '九品', 阶段: '初期', 最短月数: 3, 标准月数: 12, 最长月数: 36 },
  { 职级名称: '九品', 阶段: '中期', 最短月数: 6, 标准月数: 24, 最长月数: 60 },
  { 职级名称: '九品', 阶段: '后期', 最短月数: 12, 标准月数: 36, 最长月数: 120 },
  { 职级名称: '九品', 阶段: '圆满', 最短月数: 24, 标准月数: 60, 最长月数: 240 },
  // 八品
  { 职级名称: '八品', 阶段: '初期', 最短月数: 12, 标准月数: 60, 最长月数: 180 },
  { 职级名称: '八品', 阶段: '中期', 最短月数: 24, 标准月数: 96, 最长月数: 300 },
  { 职级名称: '八品', 阶段: '后期', 最短月数: 36, 标准月数: 144, 最长月数: 480 },
  { 职级名称: '八品', 阶段: '圆满', 最短月数: 60, 标准月数: 240, 最长月数: 720 },
  // 七品
  { 职级名称: '七品', 阶段: '初期', 最短月数: 60, 标准月数: 240, 最长月数: 600 },
  { 职级名称: '七品', 阶段: '中期', 最短月数: 120, 标准月数: 360, 最长月数: 960 },
  { 职级名称: '七品', 阶段: '后期', 最短月数: 180, 标准月数: 480, 最长月数: 1200 },
  { 职级名称: '七品', 阶段: '圆满', 最短月数: 240, 标准月数: 720, 最长月数: 1800 },
];
...
/** 策略品质到加成的映射 */
const STRATEGY_QUALITY_BONUS: Record<string, number> = {
  '凡': 0.0,
  '黄': 0.1,
  '玄': 0.25,
  '地': 0.45,
...
/** 行政效率计算输入参数 */
export interface AdministrationEfficiencyInput {
  // 位置信息
  民心基础值: number;  // 1-100
...
/** 基础行政效率（每回合政绩增加） */
const BASE_ADMINISTRATION_EFFICIENCY = 1;

/**
 * 计算综合行政效率
 */
export function calculateAdministrationEfficiency(input: AdministrationEfficiencyInput): AdministrationEfficiencyResult {
  // 1. 计算民心基础值系数
  const publicTrustBaseFactor = calculatePublicTrustBaseFactor(input.民心基础值);
  ...
  // 4. 计算策略加成
  const strategyBonus = input.策略品质
    ? calculateStrategyBonus(input.策略品质, input.行政进度 ?? 0)
    : 0;
  ...
  // 6. 综合计算
  // 公式：基础速度 × 民心系数 × 六司综合系数 × 状态系数 × (1 + 策略加成 + 环境加成)
  const combinedFactor =
    publicTrustBaseFactor *
    sixSiResult.combined *
    statusFactor *
    (1 + strategyBonus + environmentBonus);

  const finalEfficiency = BASE_ADMINISTRATION_EFFICIENCY * combinedFactor;

  // 7. 预估晋升时间
  const remainingProgress = input.下一级所需 - input.当前进度;
  const estimatedPromotionTime = estimatePromotionTime(
    input.当前职级,
    input.当前阶段,
    remainingProgress,
    finalEfficiency
  ...
  return {
    基础效率: BASE_ADMINISTRATION_EFFICIENCY,
    综合系数: combinedFactor,
    最终效率: finalEfficiency,
    预计晋升时间: estimatedPromotionTime,
    因子详情: {
      民心基础值系数: publicTrustBaseFactor,
      先天六司系数: sixSiResult.innate,
      后天六司系数: sixSiResult.acquired,
      状态效果系数: statusFactor,
      策略加成系数: strategyBonus,
...
function estimatePromotionTime(
  ...
  if (efficiency <= 0 || remainingProgress <= 0) {
    return '已可晋升';
  }

  // 查找当前职级阶段的标准时间
  const standard = RANK_PROMOTION_STANDARDS.find(
    (s) => s.职级名称 === rank && s.阶段 === stage
  );

  if (!standard) {
    // 未找到标准，使用简单计算
    const rounds = Math.ceil(remainingProgress / efficiency);
    return `约${rounds}回合`;
  }

  // 基于标准时间和当前效率计算
  // 假设标准效率为1.0时需要标准月数
  const adjustedMonths = Math.ceil(standard.标准月数 * (remainingProgress / 100) / efficiency);
...
export function getPromotionStandard(
  rank: string,
  stage: string
): InternalPromotionTime | undefined {
  return RANK_PROMOTION_STANDARDS.find(
    (s) => s.职级名称 === rank && s.阶段 === stage
  );
}
 ...
 * @returns 是否合理，以及原因
 */
export function validateAdministrationProgress(
  rank: string,
  stage: string,
  progressGain: number,
  ...
  const standard = getPromotionStandard(rank, stage);
  ...
  // 计算理论上的最大进度增长
  // 假设100进度需要标准月数，则每月最大进度 = 100 / 最短月数 * efficiency
  const maxProgressPerMonth = (100 / standard.最短月数) * efficiency;
...
export { SIX_SI_WEIGHTS, PUBLIC_TRUST_RANGES };
