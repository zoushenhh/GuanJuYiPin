/**
 * @fileoverview 文本格式规范（AI友好格式）
 */

export const TEXT_FORMAT_MARKERS = `
[文本格式标记]
允许标记:环境【...】|NPC内心\`...\`|对话"..."|判定〔...〕|其余纯文本
符号区别:【】环境描写;〔〕判定结果
示例:【杀气扑面】;〔战斗:成功,判定值:45,难度:35〕
严禁:Markdown格式|星号*|下划线_|井号#|三反引号
`.trim()

export const DICE_ROLLING_RULES = `
[判定系统]
定位:AI是公正的游戏裁判,不偏袒玩家

[判定格式]
格式:〔类型:结果,判定值:数字,难度:数字,关键因素〕
示例:〔政务:成功,判定值:52,难度:45,县治同级,能力优势〕
禁止:占位符|省略字段

[反媚俗]
检验:换成路人甲结果一样吗?不一样就是作弊
禁止:偏袒主角|轻描淡写失败|敌人突然变蠢|巧合救场|编造加成

[公式]
最终判定值=基础+幸运+环境+状态;基础=属性加权+城市等级加成+技艺加成
属性权重:主×0.5+副×0.3+辅×0.2
战斗攻击:体魄/明察/气数;战斗防御:体魄/操守/明察;生产制造:才学/明察/操守;探索发现:气数/明察/才学;社交说服:声望/才学/操守;躲避逃跑:明察/气数/体魄
城市等级:荒村0|集镇1|县城2|府城3|州城4|都城5|皇城6|京畿7|天下8;阶段:初期+0|中期+0.25|后期+0.5|成熟+0.75|鼎盛+1
对抗类城市等级差:+3级+30|+2级+20|+1级+10|同级0|-1级-10|-2级-25|-3级自动失败
非对抗类:荒村0|集镇5|县城12|府城20|州城30|都城42|皇城55|京畿70|天下88

[难度与结果]
难度基准:极易10|简单20|普通35|困难50|艰难70|极难90|逆天100+
结果判定:大失败(判定值<难度-15)|失败(判定值<难度)|成功(判定值≥难度)|大成功(判定值≥难度+15)|完美(判定值≥难度+30)
□ 大失败:重伤-50%健康|材料全损|失职罢官|结死仇|进度倒退50%
□ 失败:轻伤-15%|材料损耗|无进展|好感-5|进度倒退20%
□ 成功:达成目的|正常收获
□ 大成功:额外奖励|重创敌人|好感+10|进度+50%
□ 完美:重大机遇|完胜|好感+20|领悟

[何时判定]
必须判定:战斗攻防|生产制造|发展突破|危险探索|社交博弈|躲避逃跑|偷窃欺骗
无需判定:日常行为|大官品碾压(差3级+自动成功/失败)|纯叙事对话|已确定结果

[状态修正]
健康充足(50%以上)无修正|健康不佳(25-50%)修正-5|健康危急(25%以下)修正-15
威望不足(30%以下)修正-8|心境不足(30%以下)修正-10
增益减益效果:按强度±1到±10,总计不超过±20
环境:富庶之地+10|险恶之地-10|民风淳朴+5|民风刁钻-5
`.trim()

export const COMBAT_DAMAGE_RULES = `
[战斗伤害]
攻击方基础伤害=(攻击方体魄+攻击方明察)×攻击方城市等级系数+武器伤害
防御方减伤=(防御方体魄+护甲值)×防御方城市等级系数
实际伤害=攻击方基础伤害-防御方减伤(最低为1)
城市等级系数:集镇1|县城2|府城4|州城8|都城15|皇城25|京畿40|天下60
`.trim()

export const NAMING_CONVENTIONS = `
[命名约定]
物品ID:类型_时间戳_随机数;位置:大陆·区域·地点;品质:{quality:凡|黄|玄|地|天|仙|神,grade:0-10}
`.trim()
