...
示例:〔战斗:成功,判定值:52,难度:45,城市等级同级,装备优势〕
禁止:占位符|省略字段
...
[公式]
最终判定值=基础+幸运+环境+状态;基础=属性加权+城市等级加成+技艺加成
属性权重:主×0.5+副×0.3+辅×0.2
战斗攻击:根骨/灵性/气运;战斗防御:根骨/心性/灵性;生产制造:悟性/灵性/心性;探索发现:气运/灵性/悟性;社交说服:魅力/悟性/心性;躲避逃跑:灵性/气运/根骨
城市等级:荒村0|集镇1|县城2|府城3|州城4|都城5|皇城6|京畿7|天下8;阶段:初期+0|中期+0.25|后期+0.5|圆满+0.75|极境+1
对抗类城市等级差:+3级+30|+2级+20|+1级+10|同级0|-1级-10|-2级-25|-3级自动失败
非对抗类:荒村0|集镇5|县城12|府城20|州城30|都城42|皇城55|京畿70|天下88

[难度与结果]
难度基准:极易10|简单20|普通35|困难50|艰难70|极难90|逆天100+
结果判定:大失败(判定值<难度-15)|失败(判定值<难度)|成功(判定值≥难度)|大成功(判定值≥难度+15)|完美(判定值≥难度+30)
□ 大失败:重伤-50%健康|材料全损|失职罢官|结死仇|进度倒退50%
...
[何时判定]
必须判定:战斗攻防|生产制造|发展突破|危险探索|社交博弈|躲避逃跑|偷窃欺骗
无需判定:日常行为|大境界碾压(差3级+自动成功/失败)|纯叙事对话|已确定结果

[状态修正]
健康充足(50%以上)无修正|健康不佳(25-50%)修正-5|健康危急(25%以下)修正-15
威望不足(30%以下)修正-8|心境不足(30%以下)修正-10
增益减益效果:按强度±1到±10,总计不超过±20
环境:福地+10|险地-10|民风淳朴+5|民风刁钻-5
`.trim()

export const COMBAT_DAMAGE_RULES = `
[战斗伤害]
攻击方基础伤害=(攻击方根骨+攻击方灵性)×攻击方城市等级系数+武器伤害
防御方减伤=(防御方根骨+护甲值)×防御方城市等级系数
