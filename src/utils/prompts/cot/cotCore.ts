...
□ 小阶段突破 → set \`角色.属性.城市等级.阶段\`（初期→中期→后期→圆满→极境）
□ 大城市等级突破 → set \`角色.属性.城市等级.名称\` + 更新属性上限

### 渡劫系统（重要！）
□ 渡劫开始 → push \`角色.效果\` 添加"渡劫中"状态
□ 每道天雷 → add \`角色.属性.气血.当前\`（负）+ add \`角色.属性.灵气.当前\`（负）
□ 渡劫成功 → set 新城市等级 + 更新属性上限 + push \`社交.事件.事件记录\`
...
□ NPC属性变化（重要！）：
  ...
  - NPC城市等级变化/突破 → set \`社交.关系.[NPC名].城市等级\` {"名称":"...","阶段":"..."}（必要时 add 寿元上限）
