...
【世界观核心理念】
- 县令模拟器采用古代官场背景，聚焦于施政、治理、官场博弈
- 严格按照用户的选择和世界设定来生成内容
- 如果用户指定了特定风格（科幻、现代、赛博朋克等），必须完全遵循该风格
- 不要预设固定的剧情方向，让用户自己决定故事走向
...
- rarity (数字): 1-5之间的整数
- attribute_modifiers (对象): 先天六资加成，总和不超过5点。属性名称必须是：体魄、才学、明察、气数、声望、操守
...
{
  "name": "儒生世家",
  "description": "出身书香门第，自幼饱读诗书，对经史子集有深厚造诣...",
  "talent_cost": 2,
  "rarity": 3,
  "attribute_modifiers": {
    "体魄": -1,
  ...
  "effects": ["书卷气：在文官群体中好感度提升", "科举加成：考试成功率提升"]
}
\`\`\`
`;
...
- 名称 (字符串): 方略名称，2-8字
- 类型 (字符串): 固定值"方略"
- 品质 (对象): {quality: "凡|下|中|上|极|卓|超", grade: 0-10}
- 数量 (数字): 固定为1
- 描述 (字符串): 100-300字的介绍
...
【品质等级规则】
- **默认生成凡品或下品**。
- 只有在剧情明确需要"珍贵策略"时才生成**中品**。
- **上品及以上严禁随意生成**，除非有"特殊机遇"等剧情。
- **超凡级完全禁止生成**。
- **基础方略名称** (如"基础施政") 必须对应**低品质** (凡/下)。

【策略技能规则】
- **数量**: 必须包含2-5个技能，不能为空数组。
- **格式**: 每个技能必须包含：技能名称(字符串)、技能描述(字符串)、熟练度要求(数字0-100)
- **解锁**: 第一个技能的熟练度要求必须为0（开局即可使用）。
  - **世界观**: 技能名称和消耗必须与世界观适配，只能使用 精力/智慧/健康/任期 四种消耗；禁止出现"活力/体力/能量"等其他资源。
- **示例**: [{"技能名称":"基础施政","技能描述":"集中精力处理政务","熟练度要求":0},{"技能名称":"明镜断案","技能描述":"洞察案情真相","熟练度要求":30}]
`;
