...
{
  "name": "过目不忘",
  "description": "天生记忆力超群，看过的策略、阵法过目不忘，学习速度大幅提升"
}
\`\`\`
`;
...
- 数量 (数字): 固定为1
- 描述 (字符串): 100-300字的介绍
- 策略效果 (对象): 包含修炼速度加成、属性加成、特殊能力
- 策略技能 (数组): 包含技能名称、描述、消耗、解锁熟练度
- 修炼进度 (数字): 初始为0
- 已装备 (布尔): 初始为false
...
- **神品完全禁止生成**。
- **基础策略名称** (如"引气诀") 必须对应**低品质** (凡/黄)。

【策略技能规则】
- **数量**: 必须包含2-5个技能，不能为空数组。
- **格式**: 每个技能必须包含：技能名称(字符串)、技能描述(字符串)、熟练度要求(数字0-100)
- **解锁**: 第一个技能的熟练度要求必须为0（开局即可使用）。
  - **世界观**: 技能名称和消耗必须与世界观适配，只能使用 活力/神识/气血/寿元 四种消耗；禁止出现"精力/体力/能量"等其他资源。
- **示例**: [{"技能名称":"基础剑气","技能描述":"凝聚灵力形成剑气攻击","熟练度要求":0},{"技能名称":"御剑术","技能描述":"使用专业技能攻敌","熟练度要求":30}]
`;
